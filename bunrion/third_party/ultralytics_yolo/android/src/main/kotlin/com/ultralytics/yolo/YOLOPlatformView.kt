...
class YOLOPlatformView(
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        try {
            when (call.method) {
                ...
                "stop" -> {
                    // Treat Flutter-side stop requests as a camera pause for this app.
                    // The full YOLOView.stop() path closes LiteRT predictor resources,
                    // which has been observed to crash after leaving the live view.
    ...
    override fun dispose() {
        ...
        try {
            // Avoid calling YOLOView.stop() from Flutter platform-view disposal.
            // On some Android/LiteRT device combinations, closing the active
            // predictor while CameraX is tearing down can crash the process a
            // few seconds after navigating away from YOLOView. Pausing unbinds
            // camera use-cases and lets the Activity dispose the platform view
            // without entering the native predictor close path.
            yoloView.pauseCamera()
